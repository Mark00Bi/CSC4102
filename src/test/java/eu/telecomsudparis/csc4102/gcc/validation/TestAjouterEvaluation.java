...
class TestAjouterEvaluation {
    ...
    @Test
    @DisplayName("Test11 :Notification envoyée à la présidente lors d'une évaluation")
    void test11_NotificationEvaluation() throws Exception {
        StringBuilder result = new StringBuilder();

        // Créer un consommateur qui capte les notifications dans result
        MonConsommateur consumer = new MonConsommateur() {
            @Override
        ...
        // Abonnement du presidente au consommateur
        
        presidente.subscribe(consumer);

        // Ajouter une évaluation
        gcc.ajouterEvaluation("c1", "e1", Avis.ACCEPTATION_FAIBLE, "rapport test",LocalDate.now());

        // Laisser du temps pour la notification asynchrone
        Thread.sleep(100);

        // Vérification
        assertTrue(result.toString().contains("Nouvelle évaluation"),
            "La présidente doit être notifiée de l'évaluation.");
    }
