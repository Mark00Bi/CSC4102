...
class TestSoumettreCommunication {
    ...
    @Test
    @DisplayName("Test13 : Notification envoyée lors de la soumission d'une communication")
    void test13_NotificationSoumissionCommunication() throws Exception {
        ...
        gcc.soumettreCommunication("a1", "c1", "Titre important", "Résumé", "Contenu", LocalDate.now());

        Thread.sleep(100);

        Assertions.assertTrue(result.toString().contains("Titre important"), 
        "Le message de notification devrait contenir le titre de la communication.");
        }

        @Test
        @DisplayName("Notification envoyée à la présidente lors de la soumission")
        void test14_NotificationSoumissionPresidente() throws Exception {
        StringBuilder result = new StringBuilder();
        ...
        // Créer présidente avec subscriber
        gcc.ajouterPresidente("p1", "Julie", "Durand", "TSP");
        Presidente pres = (Presidente) gcc.utilisateurs().get("p1");
        pres.subscribe(customConsumer);

        // Soumettre communication
        gcc.soumettreCommunication("a1", "c2", "Titre", "Résumé", "Contenu", LocalDate.now());

        Thread.sleep(100); // attendre notification

        Assertions.assertTrue(result.toString().contains("Nouvelle communication"),
                "La présidente doit recevoir une notification.");
        }
